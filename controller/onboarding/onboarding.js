...
const onboardStep1 = async (req, res) => {
    ...
    try {
      ...
      if (!userCosts) {
        ...
        updates.forEach(({ productId, cost }) => {
          const existing = userCosts.products.find(p => p.productId === productId);
          if (existing) {
            existing.cost = cost;
      ...
      user.step = 4;
      await user.save();
      res.status(200).json({ message: "Costs updated successfully",nextStep: 4 });
    } catch (err) {
  ...
  const onboardStep4 = async (req, res) => {
    ...
    if (!admin || !admin.metaAccessToken) {
      return res.status(401).json({ message: "Meta access token not found. Please contact the developer." });
    }
    ...
    try {
      const testMetaResponse = await axios.get(`https://graph.facebook.com/v12.0/${adAccountId}`, {
        headers: {
      ...
      console.error("Error verifying Meta credentials:", error.response?.data || error.message);
      return res.status(401).json({ message: "Invalid Meta credentials" });
    }
  ...
  const onboardStep5 = async (req, res) => {
    ...
    try{
      ...
      const verifyToken=await axios.get("https://apiv2.shiprocket.in/v1/external/orders", {
        headers: {
          "Content-Type": "application/json",
      ...
      if(verifyToken.status !== 200 || !verifyToken.data) {
        return res.status(401).json({ message: "Invalid Shiprocket credentials" });
      }
...
module.exports = { currentStep,onboardStep1,onboardStep2,onboardStep4,onboardStep5,fetchproduct,manufacture };
