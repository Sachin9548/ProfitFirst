...
const defaultSpendData = [
  ...
  { name: 'Campaign E', spend: 11000, cpp: 8000, roas: 2.8 },
  { name: 'Campaign F', spend: 25000, cpp: 2000, roas: 5.8 },
];
...
const defaultAnalysisData = [
  { name: 'Campaign A', spend: 15000, cpp: 50, roas: 4.5, status: 'Performing Well' },
  { name: 'Campaign B', spend: 12000, cpp: 60, roas: 3.8, status: 'Average' },
  { name: 'Campaign C', spend: 9000, cpp: 55, roas: 2.5, status: 'Average' },
  { name: 'Campaign D', spend: 8000, cpp: 70, roas: 1.8, status: 'Needs Attention' },
];
...
const Marketing = () => {
  ...
  const [spendData, setSpendData] = useState(defaultSpendData);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setMetaCampaignMetrics(
          campaignMetrics && Object.keys(campaignMetrics).length
            ? campaignMetrics
            : defaultMetaCampaignMetrics
        );
        setSpendData(spendChartData?.length ? spendChartData : defaultSpendData);
        setMetaAdsData(adsChartData?.length ? adsChartData : defaultMetaAdsData);
        setDetailedAnalysisData(
          analysisTable?.length ? analysisTable : defaultAnalysisData
        );
      ...
      <div className="bg-[#161616] rounded-xl p-4 z-1">
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full text-md">
            ...
            <tbody>
  {(detailedAnalysisData.length ? detailedAnalysisData : defaultAnalysisData).map((row, idx) => (
    <tr key={idx} className="border-t border-gray-700">
      ...
      <td className={`p-2 ${
        row.status === 'Performing Well' ? 'text-green-400' :
        row.status === 'Average' ? 'text-yellow-400' :
        'text-red-400'
      }`}>{row.status}</td>
    </tr>
  ))}
...
>
  ...
  {selectedCampaign ? `${selectedCampaign} Breakdown` : "All Campaigns Overview"}
</h3>
  ...
  <div className="bg-[#161616] rounded-xl p-6">
    ...
    <h4 className="text-lg font-semibold text-green-400">{metrics.roas}</h4>
  </div>
</div>
    </div>
  );
